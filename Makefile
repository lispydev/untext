...
# TODO: rename to .so if this doesn't break pyinstaller
# the cpython-313-x86_64-linux-gnu tag is generated by python to isolate versions
so := $(src:.py=.cpython-313-x86_64-linux-gnu.so)
#so := $(src:.py=.so)

src-cython/untext/%.cpython-313-x86_64-linux-gnu.so: src-cython/untext/%.py
	./build_venv/bin/cythonize -i $< #-3
	# TODO: find the file and mv it
	ls $(basename $<).*.so
...
clean-cython:
	#find src-cython/untext/ -name "*.c" -delete
	#find src-cython/untext/ -name "*.so" -delete
	rm -r src-cython
